/**
 * edge++ — Enhanced edgetunnel with User Management
 * Core: edgetunnel by cmliu (https://github.com/cmliu/edgetunnel)
 * User Management: Inspired by Zeus Panel (https://github.com/IR-NETLIFY/zeus)
 * 
 * Built: 2026-07-11T02:47:04.190Z
 * Platform: Cloudflare Workers / Pages
 * Bindings: KV (edgetunnel settings) + D1 (user management)
 */

///////////////////////////////////////////////////////
// edge++ User Management Layer
// D1-based user management inspired by Zeus Panel
///////////////////////////////////////////////////////
...
async function ensureEdgePPSchema(db) {
  ...
  try {
    await db.prepare("CREATE TABLE IF NOT EXISTS edgepp_settings (key TEXT PRIMARY KEY, value TEXT)").run();
  } catch (e) { /* table exists */ }
  try {
    await db.prepare("CREATE TABLE IF NOT EXISTS request_log (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, ip TEXT, path TEXT, timestamp INTEGER DEFAULT (strftime('%s','now')))").run();
  } catch (e) { /* table exists */ }
  schemaEnsured = true;
}
...
async function getPanelPassword(db) {
  ...
  try {
    const row = await db.prepare("SELECT value FROM edgepp_settings WHERE key = 'panel_password'").first();
    cachedPanelPassword = row ? row.value : null;
...
async function verifyPanelAuth(request, env) {
  ...
  if (authCookie) {
    ...
    return val === await sha256(password + 'edgepp_salt');
  }
  return false;
}
...
function getActiveIpCount(activeIps) {
  ...
  try {
    const data = JSON.parse(activeIps);
    return Object.keys(data).length;
  } catch { return 0; }
}

function checkUserLimits(user) {
  if (user.is_active !== 1) return { allowed: false, reason: 'حساب غیرفعال است' };
  if (user.limit_gb) {
    const rem = user.limit_gb - (user.used_gb || 0);
    if (rem <= 0) return { allowed: false, reason: 'محدودیت حجم تمام شده' };
  }
  if (user.expiry_days && user.created_at) {
    const created = new Date(user.created_at + 'Z');
    const expiryDate = new Date(created.getTime() + user.expiry_days * 86400000);
    if (Date.now() > expiryDate.getTime()) return { allowed: false, reason: 'حساب منقضی شده' };
  }
  if (user.limit_req && user.used_req >= user.limit_req) return { allowed: false, reason: 'محدودیت درخواست تمام شده' };
  return { allowed: true };
}
...
async function handlePanelRequest(request, env) {
  if (!env.DB) {
    return new Response('D1 Database not bound. Please bind a D1 database as "DB".', { status: 500 });
  }
  const hasPassword = await getPanelPassword(env.DB);
  if (!hasPassword) {
    return new Response(HTML_PANELS.setup, {
      headers: { 'Content-Type': 'text/html; charset=utf-8' }
    });
  }
  const authorized = await verifyPanelAuth(request, env);
  if (!authorized) {
  ...
  return new Response(HTML_PANELS.panel, {
    headers: {
...
async function handlePanelLogin(request, env) {
  ...
  if (request.method === 'POST') {
    try {
      ...
      if (password === hasPassword) {
        ...
        return new Response(JSON.stringify({ success: true }), {
          ...
          headers: {
            ...
            'Set-Cookie': `edgepp_auth=${hash}; Path=/panel; Max-Age=86400; HttpOnly; Secure; SameSite=Strict`
          }
        });
      }
      ...
      return new Response(JSON.stringify({ error: 'درخواست نامعتبر' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
  }
  return new Response(HTML_PANELS.login, {
    headers: { 'Content-Type': 'text/html; charset=utf-8' }
  });
}

// ============ User Status Page ============
async function handleUserStatusPage(username, env, url) {
  ...
  try {
    ...
    const userData = JSON.stringify({
      ...
      status: limitCheck.allowed ? 'active' : limitCheck.reason,
      host: host
    });

    const html = HTML_PANELS.status.replace('/* {{USER_DATA}} */', `window.__userData = ${userData};`);
...
async function handleUserSubscription(username, request, env, url) {
  if (!env.DB) return new Response('D1 not configured', { status: 500 });
  try {
    const user = await env.DB.prepare("SELECT * FROM users WHERE username = ? OR uuid = ?").bind(username, username).first();
    ...
    if (!limitCheck.allowed) {
      return new Response(`# edge++ — دسترسی محدود\n# ${limitCheck.reason}\n# لطفا با مدیر تماس بگیرید`, {
    ...
    // Track request
    try {
      await env.DB.prepare("UPDATE users SET used_req = used_req + 1, last_active = ? WHERE username = ?").bind(Date.now(), user.username).run();
    } catch (e) {}

    // Generate subscription configs based on edgetunnel settings
    const host = url.hostname;
    const uuid = user.uuid;
    const tls = user.tls || 'tls';
    const port = user.port || 443;
    const sni = user.ips || host;
    ...
    // VLESS config
    let config = '';
    const vlessLink = `vless://${uuid}@${host}:${port}?encryption=none&security=${tls}&sni=${sni}&fp=${fp}&type=${type}&path=${path}#edge++_${user.username}`;
    config += vlessLink + '\n';

    // Trojan config
    const trojanPass = uuid;
    const trojanLink = `trojan://${trojanPass}@${host}:${port}?security=${tls}&sni=${sni}&fp=${fp}&type=${type}&path=${path}#edge++_${user.username}_trojan`;
    config += trojanLink + '\n';

    // SS config (aes-256-gcm)
    const ssMethod = 'aes-256-gcm';
    const ssPass = uuid.replace(/-/g, '');
    const ssEncoded = btoa(`${ssMethod}:${ssPass}`);
    const ssLink = `ss://${ssEncoded}@${host}:${port}?security=${tls}&sni=${sni}&fp=${fp}&type=${type}&path=${path}#edge++_${user.username}_ss`;
    config += ssLink + '\n';
    ...
    const headers = {
      ...
      'Profile-web-page-url': url.protocol + '//' + url.host + '/status/' + encodeURIComponent(user.username),
    };

    const totalGB = user.limit_gb ? user.limit_gb * 1024 * 1024 * 1024 : 0;
    const usedGB = (user.used_gb || 0) * 1024 * 1024 * 1024;
    if (totalGB > 0) {
      const expiry = user.expiry_days && user.created_at
        ? Math.floor(new Date(user.created_at + 'Z').getTime() / 1000) + user.expiry_days * 86400
        : 4102329600;
      headers['Subscription-Userinfo'] = `upload=${Math.floor(usedGB * 0.3)}; download=${Math.floor(usedGB * 0.7)}; total=${totalGB}; expire=${expiry}`;
    }

    return new Response(config.trim(), { headers });
  } catch (err) {
...
async function handleEdgePPApi(request, env, ctx, url) {
  ...
  if (path === 'setup-password' && request.method === 'POST') {
    const hasPassword = await getPanelPassword(env.DB);
    if (hasPassword) {
      return new Response(JSON.stringify({ error: 'رمز عبور از قبل تنظیم شده' }), {
  ...
  if (path === 'users' && request.method === 'GET') {
    try {
      ...
      const result = users.results.map(u => ({
        ...u,
        online_count: getActiveIpCount(u.active_ips),
        status: checkUserLimits(u)
      }));
  ...
  if (path === 'stats' && request.method === 'GET') {
    try {
      ...
      const totalUsedGB = await env.DB.prepare("SELECT COALESCE(SUM(used_gb), 0) as total FROM users").first();
      const totalUsedReq = await env.DB.prepare("SELECT COALESCE(SUM(used_req), 0) as total FROM users").first();
      const onlineNow = await env.DB.prepare("SELECT COUNT(*) as count FROM users WHERE active_ips IS NOT NULL AND active_ips != ''").first();
      return new Response(JSON.stringify({
        total_users: totalUsers.count,
        active_users: activeUsers.count,
        total_used_gb: totalUsedGB.total,
        total_used_req: totalUsedReq.total,
        online_now: onlineNow.count
      }), {
        status: 200, headers: { 'Content-Type': 'application/json' }
      });
    } catch (err) {
  ...
  if (path === 'users' && request.method === 'POST') {
    try {
      ...
      const { username, uuid, limit_gb, expiry_days, limit_req, ips, tls, port, fingerprint, ip_limit } = body;
      if (!username) return new Response(JSON.stringify({ error: 'نام کاربری الزامی است' }), { status: 400, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
      const finalUuid = uuid || generateUUID();
      const now = new Date().toISOString().replace('T', ' ').split('.')[0];
      await env.DB.prepare(
        "INSERT INTO users (username, uuid, limit_gb, expiry_days, limit_req, ips, tls, port, fingerprint, ip_limit, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      ).bind(
        username, finalUuid,
      ...
      return new Response(JSON.stringify({ success: true, uuid: finalUuid }), {
        status: 200, headers: { 'Content-Type': 'application/json' }
      });
    } catch (err) {
      const msg = err.message.includes('UNIQUE') ? 'نام کاربری تکراری است' : err.message;
  ...
  if (path.startsWith('users/') && path.endsWith('/toggle') && request.method === 'POST') {
    const username = decodeURIComponent(path.replace('users/', '').replace('/toggle', ''));
    try {
  ...
  // Reset user usage
  if (path.startsWith('users/') && path.endsWith('/reset') && request.method === 'POST') {
    const username = decodeURIComponent(path.replace('users/', '').replace('/reset', ''));
    try {
      await env.DB.prepare("UPDATE users SET used_gb = 0, used_req = 0 WHERE username = ?").bind(username).run();
      return new Response(JSON.stringify({ success: true }), {
        status: 200, headers: { 'Content-Type': 'application/json' }
      });
    } catch (err) {
  ...
  if (path.startsWith('users/') && request.method === 'PUT') {
    ...
    try {
      const body = await request.json();
      const { username: new_username, limit_gb, expiry_days, limit_req, ips, tls, port, fingerprint, ip_limit, is_active } = body;
      await env.DB.prepare(
        "UPDATE users SET username = ?, limit_gb = ?, expiry_days = ?, limit_req = ?, ips = ?, tls = ?, port = ?, fingerprint = ?, ip_limit = ?, is_active = ? WHERE username = ?"
      ).bind(
      ...
      const msg = err.message.includes('UNIQUE') ? 'نام کاربری تکراری است' : err.message;
      return new Response(JSON.stringify({ error: msg }), { status: 400, headers: { 'Content-Type': 'application/json; charset=utf-8' } });
    }
  }
  ...
  if (path === 'change-password' && request.method === 'POST') {
    try {
      ...
      const stored = await getPanelPassword(env.DB);
      ...
      if (!new_password || new_password.length < 4) {
        return new Response(JSON.stringify({ error: 'رمز جدید باید حداقل ۴ کاراکتر باشد' }), {
          status: 400, headers: { 'Content-Type': 'application/json; charset=utf-8' }
        });
      }
      await env.DB.prepare("UPDATE edgepp_settings SET value = ? WHERE key = 'panel_password'").bind(new_password).run();
      cachedPanelPassword = new_password;
      return new Response(JSON.stringify({ success: true }), {
        status: 200, headers: { 'Content-Type': 'application/json' }
      });
    } catch (err) {
    ...
    try {
      ...
      if (body.settings && typeof body.settings === 'object') {
        for (const [k, v] of Object.entries(body.settings)) {
          await env.DB.prepare("INSERT OR REPLACE INTO edgepp_settings (key, value) VALUES (?, ?)").bind(k, String(v)).run();
        }
      }
  ...
  return new Response(JSON.stringify({ error: 'API not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
}

///////////////////////////////////////////////////////
// edge++ Panel HTML Templates
// Custom unique design - Persian UI
///////////////////////////////////////////////////////
...
<title>edge++ — راه‌اندازی اولیه</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',Tahoma,sans-serif;background:#0a0e1a;min-height:100vh;display:flex;align-items:center;justify-content:center;color:#e2e8f0}
.card{background:linear-gradient(135deg,#0f172a,#1e293b);border:1px solid rgba(99,102,241,.3);border-radius:24px;padding:48px;width:420px;text-align:center;box-shadow:0 25px 60px rgba(0,0,0,.5)}
.logo{font-size:48px;margin-bottom:8px}
h1{font-size:28px;background:linear-gradient(90deg,#818cf8,#c084fc,#f472b6);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:8px}
.subtitle{color:#94a3b8;font-size:14px;margin-bottom:32px}
input{width:100%;padding:14px 20px;border-radius:14px;border:1px solid #334155;background:#1e293b;color:#e2e8f0;font-size:16px;text-align:center;outline:none;transition:.3s}
input:focus{border-color:#818cf8;box-shadow:0 0 20px rgba(99,102,241,.2)}
button{width:100%;padding:14px;border:none;border-radius:14px;background:linear-gradient(90deg,#6366f1,#a855f7);color:#fff;font-size:16px;font-weight:600;cursor:pointer;margin-top:16px;transition:.3s}
button:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(99,102,241,.4)}
.msg{margin-top:16px;color:#f87171;font-size:13px;display:none}
</style>
</head>
<body>
<div class="card">
  <div class="logo">⚡</div>
  <h1>edge++</h1>
  <p class="subtitle">تنظیم رمز عبور پنل مدیریت</p>
  <form id="setupForm">
    <input type="password" id="pwd" placeholder="رمز عبور جدید" required minlength="4">
    <button type="submit">فعال‌سازی پنل</button>
  </form>
...
document.getElementById('setupForm').onsubmit=async e=>{
  ...
  const r=await fetch('/api/edgepp/setup-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pwd})});
  const d=await r.json();
  if(d.success){location.href='/panel';}
  else{const m=document.getElementById('msg');m.textContent=d.error;m.style.display='block';}
};
</script>
</body>
</html>`;
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>edge++ — ورود</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',Tahoma,sans-serif;background:#0a0e1a;min-height:100vh;display:flex;align-items:center;justify-content:center;color:#e2e8f0}
.card{background:linear-gradient(135deg,#0f172a,#1e293b);border:1px solid rgba(99,102,241,.3);border-radius:24px;padding:48px;width:400px;text-align:center;box-shadow:0 25px 60px rgba(0,0,0,.5)}
.logo{font-size:48px;margin-bottom:8px}
h1{font-size:28px;background:linear-gradient(90deg,#818cf8,#c084fc,#f472b6);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:32px}
input{width:100%;padding:14px 20px;border-radius:14px;border:1px solid #334155;background:#1e293b;color:#e2e8f0;font-size:16px;text-align:center;outline:none;transition:.3s}
input:focus{border-color:#818cf8;box-shadow:0 0 20px rgba(99,102,241,.2)}
button{width:100%;padding:14px;border:none;border-radius:14px;background:linear-gradient(90deg,#6366f1,#a855f7);color:#fff;font-size:16px;font-weight:600;cursor:pointer;margin-top:16px;transition:.3s}
button:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(99,102,241,.4)}
...
<div class="card">
  ...
  <h1>edge++</h1>
  <form id="loginForm">
    <input type="password" id="pwd" placeholder="رمز عبور" required>
...
document.getElementById('loginForm').onsubmit=async e=>{
  ...
  const d=await r.json();
  if(d.success){location.href='/panel';}
  else{const m=document.getElementById('msg');m.textContent=d.error||'خطا در ورود';m.style.display='block';}
};
</script>
</body>
</html>`;
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>edge++ — پنل مدیریت</title>
<style>
:root{--bg-primary:#0a0e1a;--bg-card:#0f172a;--bg-card2:#1e293b;--border:rgba(99,102,241,.15);--accent:#818cf8;--accent2:#a855f7;--accent3:#f472b6;--text:#e2e8f0;--text2:#94a3b8;--success:#34d399;--danger:#f87171;--warning:#fbbf24}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',Tahoma,sans-serif;background:var(--bg-primary);color:var(--text);min-height:100vh}
/* Scrollbar */
::-webkit-scrollbar{width:6px}
::-webkit-scrollbar-track{background:var(--bg-primary)}
::-webkit-scrollbar-thumb{background:#334155;border-radius:3px}
/* Header */
.header{background:linear-gradient(90deg,rgba(15,23,42,.95),rgba(30,41,59,.95));border-bottom:1px solid var(--border);padding:16px 32px;display:flex;align-items:center;justify-content:space-between;backdrop-filter:blur(20px);position:sticky;top:0;z-index:100}
.header-right{display:flex;align-items:center;gap:16px}
.logo{font-size:28px}.brand{font-size:20px;font-weight:700;background:linear-gradient(90deg,var(--accent),var(--accent2),var(--accent3));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.header-left{display:flex;align-items:center;gap:12px}
.header-btn{padding:8px 18px;border-radius:10px;border:1px solid var(--border);background:transparent;color:var(--text2);cursor:pointer;font-size:13px;transition:.3s}
...
.stat-card{background:var(--bg-card);border:1px solid var(--border);border-radius:16px;padding:24px;position:relative;overflow:hidden;transition:.3s}
...
.add-btn{padding:10px 24px;border:none;border-radius:10px;background:linear-gradient(90deg,var(--accent),var(--accent2));color:#fff;font-size:14px;font-weight:600;cursor:pointer;transition:.3s}
.add-btn:hover{transform:translateY(-1px);box-shadow:0 6px 20px rgba(99,102,241,.3)}
table{width:100%;border-collapse:collapse}
th{padding:14px 16px;text-align:right;font-size:12px;color:var(--text2);font-weight:600;text-transform:uppercase;letter-spacing:.5px;border-bottom:1px solid var(--border);background:rgba(0,0,0,.2)}
td{padding:14px 16px;border-bottom:1px solid rgba(51,65,85,.5);font-size:13px;vertical-align:middle}
tr:hover td{background:rgba(99,102,241,.03)}
.badge{padding:4px 12px;border-radius:8px;font-size:11px;font-weight:600;display:inline-block}
.badge-active{background:rgba(52,211,153,.1);color:var(--success);border:1px solid rgba(52,211,153,.2)}
.badge-inactive{background:rgba(248,113,113,.1);color:var(--danger);border:1px solid rgba(248,113,113,.2)}
.badge-limited{background:rgba(251,191,36,.1);color:var(--warning);border:1px solid rgba(251,191,36,.2)}
.uuid{font-family:monospace;font-size:11px;color:var(--text2);direction:ltr;text-align:right}
.actions{display:flex;gap:6px}
.act-btn{padding:5px 10px;border-radius:8px;border:1px solid var(--border);background:transparent;color:var(--text2);cursor:pointer;font-size:11px;transition:.2s}
...
.modal{background:var(--bg-card);border:1px solid var(--border);border-radius:20px;padding:32px;width:90%;max-width:520px;max-height:90vh;overflow-y:auto}
.modal h2{font-size:20px;margin-bottom:24px;display:flex;align-items:center;gap:10px}
.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
.form-group{display:flex;flex-direction:column;gap:6px}
.form-group.full{grid-column:1/-1}
.form-group label{font-size:12px;color:var(--text2);font-weight:600}
.form-group input,.form-group select{padding:10px 14px;border-radius:10px;border:1px solid #334155;background:var(--bg-card2);color:var(--text);font-size:14px;outline:none;transition:.3s;direction:rtl}
.form-group input:focus,.form-group select:focus{border-color:var(--accent)}
.modal-actions{display:flex;gap:10px;margin-top:24px;justify-content:flex-start}
.btn{padding:10px 24px;border-radius:10px;border:none;font-size:14px;font-weight:600;cursor:pointer;transition:.3s}
.btn-primary{background:linear-gradient(90deg,var(--accent),var(--accent2));color:#fff}
.btn-primary:hover{box-shadow:0 6px 20px rgba(99,102,241,.3)}
.btn-ghost{background:transparent;border:1px solid var(--border);color:var(--text2)}
.btn-ghost:hover{border-color:var(--text2)}
/* Copy toast */
.toast{position:fixed;bottom:24px;left:24px;padding:12px 24px;background:var(--bg-card2);border:1px solid var(--border);border-radius:12px;color:var(--success);font-size:13px;transform:translateY(80px);opacity:0;transition:.3s;z-index:300}
.toast.show{transform:translateY(0);opacity:1}
/* Search */
.search{padding:8px 16px;border-radius:10px;border:1px solid #334155;background:var(--bg-card2);color:var(--text);font-size:13px;outline:none;width:240px;direction:rtl}
...
.stats .stat-card,.table-wrapper{animation:fadeIn .4s ease}
.stats .stat-card:nth-child(2){animation-delay:.05s}
.stats .stat-card:nth-child(3){animation-delay:.1s}
.stats .stat-card:nth-child(4){animation-delay:.15s}
</style>
</head>
<body>
...
<div class="header">
  ...
  <div class="header-left">
    <a href="/admin" class="header-btn" target="_blank">پنل edgetunnel</a>
    <button class="header-btn" onclick="openChangePassword()">تغییر رمز</button>
    <button class="header-btn btn-danger" onclick="logout()">خروج</button>
  </div>
</div>
  ...
  <div class="table-wrapper">
    <div class="table-header">
      <div style="display:flex;align-items:center;gap:16px">
        <span class="table-title">مدیریت کاربران</span>
        <input class="search" id="searchInput" placeholder="جستجو..." oninput="renderUsers()">
      </div>
      <button class="add-btn" onclick="openAddUser()">+ افزودن کاربر</button>
    </div>
...
<div class="modal-overlay" id="userModal">
  <div class="modal">
    <h2 id="modalTitle">👤 افزودن کاربر</h2>
    <input type="hidden" id="editUsername">
    <div class="form-grid">
      <div class="form-group">
        <label>نام کاربری *</label>
        <input id="f_username" placeholder="مثال: user1" required>
      </div>
      <div class="form-group">
        <label>UUID</label>
        <input id="f_uuid" placeholder="خالی = خودکار" dir="ltr">
      </div>
      <div class="form-group">
        <label>محدودیت حجم (GB)</label>
        <input id="f_limit_gb" type="number" step="0.1" placeholder="خالی = نامحدود">
      </div>
      <div class="form-group">
        <label>مدت اعتبار (روز)</label>
        <input id="f_expiry_days" type="number" placeholder="خالی = نامحدود">
      </div>
      <div class="form-group">
        <label>محدودیت درخواست</label>
        <input id="f_limit_req" type="number" placeholder="خالی = نامحدود">
      </div>
      <div class="form-group">
        <label>محدودیت IP همزمان</label>
        <input id="f_ip_limit" type="number" placeholder="خالی = پیش‌فرض">
      </div>
      <div class="form-group">
        <label>پروتکل TLS</label>
        <select id="f_tls">
      ...
      <div class="form-group">
        <label>پورت</label>
        <input id="f_port" type="number" value="443" dir="ltr">
      </div>
      <div class="form-group">
        <label>فینگرپرینت</label>
        <select id="f_fingerprint">
      ...
      <div class="form-group">
        <label>SNI / آدرس سفارشی</label>
        <input id="f_ips" placeholder="خالی = دامنه فعلی" dir="ltr">
      </div>
    </div>
    <div class="modal-actions">
      <button class="btn btn-primary" onclick="saveUser()">ذخیره</button>
      <button class="btn btn-ghost" onclick="closeModal('userModal')">انصراف</button>
    </div>
  </div>
</div>
  ...
  <div class="modal">
    ...
    <div class="form-grid">
      ...
      <div class="form-group full">
        ...
        <input type="password" id="cp_new">
      </div>
    </div>
    <div class="modal-actions">
      <button class="btn btn-primary" onclick="changePassword()">تغییر</button>
      <button class="btn btn-ghost" onclick="closeModal('pwModal')">انصراف</button>
    </div>
  </div>
</div>
...
async function api(path, opts = {}) {
  const r = await fetch('/api/edgepp/' + path, {
    ...opts,
...
async function loadStats() {
  try {
    ...
    document.getElementById('stats').innerHTML = \`
      ...
      <div class="stat-card">
        <div class="stat-icon">📊</div>
        <div class="stat-value">\${s.total_used_gb.toFixed(2)} GB</div>
        <div class="stat-label">مصرف کل</div>
      </div>
      <div class="stat-card">
        <div class="stat-icon">🔗</div>
        <div class="stat-value">\${s.total_used_req.toLocaleString()}</div>
...
function renderUsers() {
  ...
  const filtered = allUsers.filter(u =>
    u.username.toLowerCase().includes(q) || u.uuid.toLowerCase().includes(q)
  );
  const tbody = document.getElementById('userTable');
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="7"><div class="empty"><div class="empty-icon">📭</div>کاربری یافت نشد</div></td></tr>';
    return;
  }
  tbody.innerHTML = filtered.map(u => {
    const status = u.status?.allowed ? 'فعال' : (u.status?.reason || 'غیرفعال');
    const badgeClass = u.is_active === 1 && u.status?.allowed ? 'active' : (u.status?.allowed ? 'limited' : 'inactive');
    const daysLeft = u.expiry_days && u.created_at
      ? Math.max(0, Math.ceil((new Date(u.created_at + 'Z').getTime() + u.expiry_days * 86400000 - Date.now()) / 86400000))
      : '∞';
    const volUsed = (u.used_gb || 0).toFixed(2);
    const volLimit = u.limit_gb ? u.limit_gb + ' GB' : '∞';
    const reqUsed = u.used_req || 0;
    const reqLimit = u.limit_req ? u.limit_req.toLocaleString() : '∞';
    const subLink = '/sub/' + encodeURIComponent(u.username);
    const statusLink = '/status/' + encodeURIComponent(u.username);
    return \`<tr>
      <td><strong>\${u.username}</strong></td>
      <td class="uuid">\${u.uuid}</td>
      <td>\${volUsed} / \${volLimit}</td>
      <td>\${reqUsed.toLocaleString()} / \${reqLimit}</td>
      <td>\${daysLeft === '∞' ? '∞' : daysLeft + ' روز'}</td>
      <td><span class="badge badge-\${badgeClass}">\${status}</span></td>
      <td>
        <div class="actions">
          <button class="act-btn" onclick="copyText('\${subLink}')">📋 اشتراک</button>
          <a href="\${statusLink}" target="_blank" class="act-btn">📊</a>
          <button class="act-btn" onclick="openEditUser('\${u.username}')">✏️</button>
          <button class="act-btn" onclick="toggleUser('\${u.username}')">\${u.is_active===1?'⏸️':'▶️'}</button>
          <button class="act-btn" onclick="resetUser('\${u.username}')">🔄</button>
          <button class="act-btn danger" onclick="deleteUser('\${u.username}')">🗑️</button>
        </div>
      </td>
    </tr>\`;
  }).join('');
...
function openAddUser() {
  document.getElementById('modalTitle').textContent = '👤 افزودن کاربر';
  document.getElementById('editUsername').value = '';
  ['f_username','f_uuid','f_limit_gb','f_expiry_days','f_limit_req','f_ip_limit','f_ips'].forEach(id => document.getElementById(id).value = '');
  document.getElementById('f_tls').value = 'tls';
  document.getElementById('f_port').value = '443';
  document.getElementById('f_fingerprint').value = 'chrome';
...
function openEditUser(username) {
  ...
  if (!u) return;
  document.getElementById('modalTitle').textContent = '✏️ ویرایش ' + username;
  document.getElementById('editUsername').value = username;
  document.getElementById('f_username').value = u.username;
  document.getElementById('f_uuid').value = u.uuid;
  document.getElementById('f_limit_gb').value = u.limit_gb || '';
  document.getElementById('f_expiry_days').value = u.expiry_days || '';
  document.getElementById('f_limit_req').value = u.limit_req || '';
  document.getElementById('f_ip_limit').value = u.ip_limit || '';
  document.getElementById('f_tls').value = u.tls || 'tls';
  document.getElementById('f_port').value = u.port || 443;
  document.getElementById('f_fingerprint').value = u.fingerprint || 'chrome';
  document.getElementById('f_ips').value = u.ips || '';
...
async function saveUser() {
  const data = {
    ...
    limit_gb: document.getElementById('f_limit_gb').value || undefined,
    expiry_days: document.getElementById('f_expiry_days').value || undefined,
    limit_req: document.getElementById('f_limit_req').value || undefined,
    ip_limit: document.getElementById('f_ip_limit').value || undefined,
    tls: document.getElementById('f_tls').value,
    port: parseInt(document.getElementById('f_port').value) || 443,
    fingerprint: document.getElementById('f_fingerprint').value,
    ips: document.getElementById('f_ips').value.trim() || undefined,
    is_active: 1
  };
  if (!data.username) return showToast('نام کاربری الزامی است', true);
  const editName = document.getElementById('editUsername').value;
  const res = editName
    ? await api('users/' + encodeURIComponent(editName), { method: 'PUT', body: JSON.stringify(data) })
    : await api('users', { method: 'POST', body: JSON.stringify(data) });
  if (res.error) return showToast(res.error, true);
  closeModal('userModal');
  await loadUsers();
  await loadStats();
  showToast(editName ? 'کاربر بروزرسانی شد' : 'کاربر اضافه شد');
}

async function toggleUser(username) {
  await api('users/' + encodeURIComponent(username) + '/toggle', { method: 'POST' });
  await loadUsers();
  await loadStats();
}

async function resetUser(username) {
  if (!confirm('مصرف کاربر ' + username + ' ریست شود؟')) return;
  await api('users/' + encodeURIComponent(username) + '/reset', { method: 'POST' });
  await loadUsers();
...
function openChangePassword() { document.getElementById('pwModal').classList.add('active'); }
...
function copyText(text) {
  const full = location.protocol + '//' + host + text;
  navigator.clipboard.writeText(full).then(() => showToast('لینک اشتراک کپی شد: ' + full));
}
...
function logout() {
  document.cookie = 'edgepp_auth=; Path=/panel; Max-Age=0';
...
// ============ User Status Page ============
HTML_PANELS.status = `<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>edge++ — وضعیت کاربر</title>
<style>
:root{--bg:#0a0e1a;--card:#0f172a;--card2:#1e293b;--border:rgba(99,102,241,.15);--accent:#818cf8;--accent2:#a855f7;--accent3:#f472b6;--text:#e2e8f0;--text2:#94a3b8;--success:#34d399;--danger:#f87171;--warning:#fbbf24}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Segoe UI',Tahoma,sans-serif;background:var(--bg);color:var(--text);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px}
.card{background:linear-gradient(135deg,var(--card),var(--card2));border:1px solid var(--border);border-radius:24px;padding:40px;width:100%;max-width:480px;text-align:center;box-shadow:0 25px 60px rgba(0,0,0,.5)}
.logo{font-size:52px;margin-bottom:4px}
h1{font-size:24px;margin-bottom:24px;background:linear-gradient(90deg,var(--accent),var(--accent2));-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:24px}
.info-item{background:rgba(0,0,0,.2);border:1px solid var(--border);border-radius:14px;padding:16px 12px}
.info-value{font-size:20px;font-weight:700;margin-bottom:2px}
.info-label{font-size:11px;color:var(--text2)}
.status-badge{display:inline-block;padding:8px 24px;border-radius:12px;font-size:14px;font-weight:600;margin-bottom:24px}
.status-active{background:rgba(52,211,153,.1);color:var(--success);border:1px solid rgba(52,211,153,.3)}
.status-inactive{background:rgba(248,113,113,.1);color:var(--danger);border:1px solid rgba(248,113,113,.3)}
.sub-link{display:inline-block;padding:12px 28px;border-radius:14px;background:linear-gradient(90deg,var(--accent),var(--accent2));color:#fff;text-decoration:none;font-weight:600;font-size:14px;transition:.3s}
.sub-link:hover{transform:translateY(-2px);box-shadow:0 8px 25px rgba(99,102,241,.3)}
.config-section{text-align:right;margin-top:24px;background:rgba(0,0,0,.2);border:1px solid var(--border);border-radius:14px;padding:16px}
.config-section h3{font-size:13px;color:var(--text2);margin-bottom:10px}
.config-box{background:var(--bg);border-radius:10px;padding:12px;font-family:monospace;font-size:11px;direction:ltr;text-align:left;word-break:break-all;max-height:120px;overflow-y:auto;color:var(--accent);line-height:1.6}
.copy-btn{margin-top:8px;padding:6px 16px;border-radius:8px;border:1px solid var(--border);background:transparent;color:var(--text2);cursor:pointer;font-size:12px}
...
<div class="card">
  <div class="logo">⚡</div>
  <h1>edge++</h1>
  <div id="statusBadge" class="status-badge status-active">در حال بارگذاری...</div>
  <div class="info-grid" id="infoGrid"></div>
  <a class="sub-link" id="subLink" href="#">📋 لینک اشتراک</a>
  <div class="config-section">
    <h3>کانفیگ‌ها</h3>
    <div class="config-box" id="configBox">در حال بارگذاری...</div>
    <button class="copy-btn" onclick="copyConfigs()">کپی کانفیگ‌ها</button>
  </div>
</div>
<script>
/* {{USER_DATA}} */
(async()=>{
  const u=window.__userData;
  if(!u){document.getElementById('statusBadge').textContent='خطا در بارگذاری';return;}
  const isActive=u.status==='active';
  const badge=document.getElementById('statusBadge');
  badge.textContent=isActive?'✅ فعال':'⛔ '+u.status;
  badge.className='status-badge '+(isActive?'status-active':'status-inactive');
  document.getElementById('infoGrid').innerHTML=\`
    <div class="info-item"><div class="info-value">\${u.username}</div><div class="info-label">نام کاربری</div></div>
    <div class="info-item"><div class="info-value">\${u.online_count}</div><div class="info-label">اتصال فعال</div></div>
    <div class="info-item"><div class="info-value">\${(u.used_gb||0).toFixed(2)} / \${u.limit_gb?u.limit_gb+' GB':'∞'}</div><div class="info-label">حجم مصرفی</div></div>
    <div class="info-item"><div class="info-value">\${(u.used_req||0).toLocaleString()} / \${u.limit_req?u.limit_req.toLocaleString():'∞'}</div><div class="info-label">درخواست‌ها</div></div>
    <div class="info-item"><div class="info-value">\${u.days_left!==null?u.days_left+' روز':'∞'}</div><div class="info-label">مانده تا انقضا</div></div>
    <div class="info-item"><div class="info-value">\${u.ip_limit||'پیش‌فرض'}</div><div class="info-label">محدودیت IP</div></div>
  \`;
  const subUrl=location.protocol+'//'+u.host+'/sub/'+encodeURIComponent(u.username);
  document.getElementById('subLink').href=subUrl;
  // Load configs
  try{
    const r=await fetch(subUrl);
    const txt=await r.text();
    document.getElementById('configBox').textContent=txt;
  }catch(e){document.getElementById('configBox').textContent='خطا در بارگذاری کانفیگ';}
})();
function copyConfigs(){
  const text=document.getElementById('configBox').textContent;
  navigator.clipboard.writeText(text).then(()=>{document.querySelector('.copy-btn').textContent='کپی شد!';setTimeout(()=>{document.querySelector('.copy-btn').textContent='کپی کانفیگ‌ها'},2000);});
}
</script>
</body>
</html>`;

// ============ edgetunnel Core (transformed) ============
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (env.DB) {
      ctx.waitUntil(ensureEdgePPSchema(env.DB));
      ctx.waitUntil(checkAutoResets(env));
    }

    // === edge++ User Management Routes ===
    // Panel routes
    if (url.pathname === '/panel' || url.pathname === '/panel/') {
      return handlePanelRequest(request, env);
    }
    ...
    if (url.pathname.startsWith('/status/')) {
      ...
      return handleUserStatusPage(username, env, url);
    }

    // Per-user subscription (with status + configs)
    if (url.pathname.startsWith('/sub/') && !url.pathname.startsWith('/sub?')) {
      const username = decodeURIComponent(url.pathname.slice(5));
      return handleUserSubscription(username, request, env, url);
    }

    // User management API
    if (url.pathname.startsWith('/api/edgepp/')) {
      return handleEdgePPApi(request, env, ctx, url);
    }

    // === Pass everything else to edgetunnel core ===
